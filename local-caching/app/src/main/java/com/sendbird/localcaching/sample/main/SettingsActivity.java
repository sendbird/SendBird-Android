...
public class SettingsActivity extends BaseActivity {
    ...
    private void saveDoNotDisturb(final boolean doNotDisturb) {
        ...
        if (doNotDisturbFrom.length() > 0 && doNotDisturbTo.length() > 0) {
            int startHour = DateUtils.getHourOfDay(Long.parseLong(doNotDisturbFrom));
            int startMin = DateUtils.getMinute(Long.parseLong(doNotDisturbFrom));
            int endHour = DateUtils.getHourOfDay(Long.parseLong(doNotDisturbTo));
            int endMin = DateUtils.getMinute(Long.parseLong(doNotDisturbTo));
    ...
    private void setDoNotDisturbTime(final boolean from, final TextView textView) {
        ...
        if (from) {
            ...
            if (doNotDisturbFrom.length() > 0) {
                timeMillis = Long.parseLong(doNotDisturbFrom);
            }
        } else {
            String doNotDisturbTo = PreferenceUtils.getNotificationsDoNotDisturbTo();
            if (doNotDisturbTo.length() > 0) {
                timeMillis = Long.parseLong(doNotDisturbTo);
            }
        }
    ...
    private void checkDoNotDisturb(boolean doNotDisturb) {
        ...
        String doNotDisturbFrom = PreferenceUtils.getNotificationsDoNotDisturbFrom();
        if (doNotDisturbFrom.length() > 0) {
            mTextViewNotificationsDoNotDisturbFrom.setText(DateUtils.formatTimeWithMarker(Long.parseLong(doNotDisturbFrom)));
        } else {
        ...
        String doNotDisturbTo = PreferenceUtils.getNotificationsDoNotDisturbTo();
        if (doNotDisturbTo.length() > 0) {
            mTextViewNotificationsDoNotDisturbTo.setText(DateUtils.formatTimeWithMarker(Long.parseLong(doNotDisturbTo)));
        } else {
    ...
    private void requestCamera() {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= 24) {
                mTempPhotoUri = FileProvider.getUriForFile(this, "com.sendbird.localcaching.sample.fileprovider", tempFile);
